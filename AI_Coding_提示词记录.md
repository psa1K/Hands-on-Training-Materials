...
### Prompt 6：按需求说明书逐项检测满足度并修复缺口

- **时间**：2026-08-22
- **提示词原文**：

> 再次阅读《Focusly番茄时钟-专注学习打卡工具》需求说明书.md具体需求，检测是否满足

- **目的**：以需求说明书 PRD（4.1-4.5、5、6 章）为准逐项核对实现，并修复不满足项。
- **核对结论**：4 大模块核心需求均满足（计时/任务/打卡/统计 + UI + 统一返回 + RESTful + 离线兜底 + loading）。发现 2 处缺口并修复：
  1. 需求 4.4「新增计时记录后图表自动刷新」未完全满足：原来只有打卡后才刷新看板 → 新增 `sessionTick`（每个专注/休息轮次完成自增），TimerPanel 监听后触发统计刷新；`api/stat.js` 新增 `mergeTodayLive`，把「今日实时专注时长」并入今日统计点，番茄完成后图表即时更新
  2. 需求 5.4「所有接口请求展示 loading」：计时配置加载无 loading → 新增 `loading` 状态并展示
- **其他说明（符合/偏差）**：
  - 接口基础路径：需求写 `/api`；因云端 Mock 实测不携带 `/api` 基础路径，前端 baseURL 使用 Mock 根地址 + 相对路径（功能正常，Apifox 项目内已设基础路径 /api）
  - 任务数据结构为需求文档超集（含 docx 新增的 description 字段）
  - 本地→Mock 的网络恢复同步为静态 Mock 限制（docx 亦承认），保留 `syncToRemote` 接口待真实后端
- **验证**：`npm run build` 通过；dev 服务器编译全部模块通过（200），改动经 HMR 生效
- **产出物**：统计看板实时自动刷新、计时配置加载态
